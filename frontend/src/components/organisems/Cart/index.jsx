...
import { fetchCart, removeFromCart } from "../../../store/action/cartAction";
...
export const Cart = ({ show }) => {
  ...
  return (
    ...
    >
      ...
      >
        <ul className="p-11 bg-white overflow-y-auto md:h-[58vh] h-[45vh] scrollbar-thin scrollbar-thumb-[#4a5568] scrollbar-track-[#cbd5e0]">
          {cartItems.map((item) => (
            <li className="flex pt-5 pb-8 border-b border-gray-200" key={item.productId}>
              <div className="w-28 mr-4">
                {item.product.listImage.length > 0 && (
                  <img
